...
ngraph::he::HEBackend::create_valued_ciphertext(
 ...
 std::shared_ptr<ngraph::he::HECiphertext> ciphertext = create_empty_ciphertext();

  encrypt(ciphertext, plaintext);
  return ciphertext;
}

shared_ptr<runtime::Tensor> ngraph::he::HEBackend::create_tensor(
...
shared_ptr<runtime::Tensor> ngraph::he::HEBackend::create_plain_tensor(
    const element::Type& element_type, const Shape& shape,
...
shared_ptr<runtime::Tensor> ngraph::he::HEBackend::create_cipher_tensor(
    const element::Type& element_type, const Shape& shape,
    const bool batched) const {
...
shared_ptr<runtime::Tensor> ngraph::he::HEBackend::create_valued_cipher_tensor(
  ...
  vector<shared_ptr<ngraph::he::HECiphertext>>& cipher_texts =
      tensor->get_elements();
...
shared_ptr<runtime::Tensor> ngraph::he::HEBackend::create_valued_plain_tensor(
    float value, const element::Type& element_type, const Shape& shape) const {
  auto tensor = static_pointer_cast<HEPlainTensor>(
      create_plain_tensor(element_type, shape));
  vector<shared_ptr<ngraph::he::HEPlaintext>>& plain_texts =
      tensor->get_elements();
#pragma omp parallel for
...
std::shared_ptr<runtime::Executable> ngraph::he::HEBackend::compile(
