...
template <class T>
shared_ptr<op::Constant> make_constant_reshape(
    shared_ptr<op::Constant> constant, shared_ptr<op::Reshape> reshape) {
  auto out_shape = reshape->get_shape();
  vector<T> out_vec(shape_size(out_shape));

  runtime::reference::reshape<T>(constant->get_vector<T>().data(),
                                 out_vec.data(), constant->get_shape(),
                                 reshape->get_input_order(), out_shape);

  return make_shared<op::Constant>(constant->get_element_type(), out_shape,
                                   out_vec);
}
...
shared_ptr<op::Constant> make_constant_pad(shared_ptr<op::Constant> constant,
  ...
  runtime::reference::pad<T>(constant->get_vector<T>().data(),
                             pad_value->get_vector<T>().data(), out_vec.data(),
                             constant->get_shape(), out_shape,
...
void pass::HEConstantFolding::construct_constant_pad() {
  auto is_constant = pattern::has_class<op::Constant>();
  auto constant_label =
      make_shared<pattern::op::Label>(element::f32, Shape{6}, is_constant);

  auto pad_value_label =
      make_shared<pattern::op::Label>(element::f32, Shape{}, is_constant);
  ...
  auto pad =
      make_shared<op::Pad>(constant_label, pad_value_label, padding_below,
  ...
  auto constant_pad_callback = [constant_label](pattern::Matcher& m) {
    ...
    auto constant_match =
        static_pointer_cast<op::Constant>(pattern_map[constant_label]);
    auto pad_match = static_pointer_cast<op::Pad>(m.get_match_root());

    auto type = constant_match->get_element_type();
    if (type == element::i32) {
      replace_node(m.get_match_root(),
                   make_constant_pad<int>(constant_match, pad_match));
      return true;
    } else if (type == element::i8) {
      replace_node(m.get_match_root(),
                   make_constant_pad<int8_t>(constant_match, pad_match));
      return true;
    } else if (type == element::f32) {
      replace_node(m.get_match_root(),
                   make_constant_pad<float>(constant_match, pad_match));
      return true;
    } else if (type == element::f64) {
      replace_node(m.get_match_root(),
                   make_constant_pad<double>(constant_match, pad_match));
      return true;
    }
  ...
  auto pad_matcher = make_shared<pattern::Matcher>(
      pad, constant_pad_callback, "HEConstantFolding.ConstantPad");
  this->add_matcher(pad_matcher);
}

void pass::HEConstantFolding::construct_constant_reshape() {
  auto constant_label = make_shared<pattern::op::Label>(
      element::f32, Shape{2, 4}, pattern::has_class<op::Constant>());
  auto reshape = make_shared<op::Reshape>(constant_label, AxisVector{0, 1},
                                          Shape{2, 4, 1});
  ...
  auto constant_reshape_callback = [constant_label](pattern::Matcher& m) {
    ...
    if (type == element::i32) {
      ...
      replace_node(m.get_match_root(), make_constant_reshape<int8_t>(
                                           constant_match, reshape_match));
      return true;
    } else if (type == element::f32) {
      replace_node(m.get_match_root(),
      ...
      replace_node(m.get_match_root(), make_constant_reshape<double>(
                                           constant_match, reshape_match));
      return true;
    }
  ...
  auto reshape_matcher = make_shared<pattern::Matcher>(
      reshape, constant_reshape_callback, "HEConstantFolding.ConstantReshape");
  this->add_matcher(reshape_matcher);
}

template <class T>
shared_ptr<op::Constant> make_constant_broadcast(
    shared_ptr<op::Constant> constant, shared_ptr<op::Broadcast> broadcast) {
  auto out_shape = broadcast->get_shape();
  vector<T> out_vec(shape_size(out_shape));

  runtime::reference::broadcast<T>(constant->get_vector<T>().data(),
                                   out_vec.data(), constant->get_shape(),
                                   out_shape, broadcast->get_broadcast_axes());

  return make_shared<op::Constant>(constant->get_element_type(), out_shape,
                                   out_vec);
}

void pass::HEConstantFolding::construct_constant_broadcast() {
  auto constant_label = make_shared<pattern::op::Label>(
      element::f32, Shape{2}, pattern::has_class<op::Constant>());

  auto broadcast =
      make_shared<op::Broadcast>(constant_label, Shape{2, 4}, AxisSet{1});
  ...
  auto constant_broadcast_callback = [constant_label](pattern::Matcher& m) {
    ...
    auto constant_match =
        static_pointer_cast<op::Constant>(pattern_map[constant_label]);
    auto broadcast_match =
        static_pointer_cast<op::Broadcast>(m.get_match_root());

    auto type = constant_match->get_element_type();
    if (type == element::i32) {
      replace_node(m.get_match_root(), make_constant_broadcast<int>(
                                           constant_match, broadcast_match));
      return true;
    } else if (type == element::i8) {
      replace_node(m.get_match_root(), make_constant_broadcast<int8_t>(
                                           constant_match, broadcast_match));
      return true;
    } else if (type == element::f32) {
      replace_node(m.get_match_root(), make_constant_broadcast<float>(
                                           constant_match, broadcast_match));
      return true;
    } else if (type == element::f64) {
      replace_node(m.get_match_root(), make_constant_broadcast<double>(
                                           constant_match, broadcast_match));
      return true;
    }
  ...
  auto broadcast_matcher =
      make_shared<pattern::Matcher>(broadcast, constant_broadcast_callback,
                                    "HEConstantFolding.ConstantBroadcast");
...
shared_ptr<op::Constant> make_constant_binary(shared_ptr<op::Constant> a,
  ...
  if (std::dynamic_pointer_cast<op::Add>(binary)) {
    runtime::reference::add<T>(a->get_vector<T>().data(),
                               b->get_vector<T>().data(), out_vec.data(),
    ...
    NGRAPH_ASSERT(false) << "make_constant_binary must be consistent with "
...
bool is_supported_binary_op(std::shared_ptr<Node> n) {
  return (std::dynamic_pointer_cast<op::Add>(n) ||
          std::dynamic_pointer_cast<op::Subtract>(n) ||
          std::dynamic_pointer_cast<op::Multiply>(n) ||
          std::dynamic_pointer_cast<op::Divide>(n) ||
          std::dynamic_pointer_cast<op::Maximum>(n) ||
          std::dynamic_pointer_cast<op::Minimum>(n));
}

void pass::HEConstantFolding::construct_constant_binary() {
  auto a = make_shared<pattern::op::Label>(element::f32, Shape{2, 4},
                                           pattern::has_class<op::Constant>());
  auto b = make_shared<pattern::op::Label>(element::f32, Shape{2, 4},
                                           pattern::has_class<op::Constant>());
  ...
  auto constant_binary_callback = [a, b](pattern::Matcher& m) {
    ...
    if (type == element::i32) {
      ...
      replace_node(m.get_match_root(), make_constant_binary<int8_t>(
                                           a_match, b_match, binary_match));
      return true;
    } else if (type == element::f32) {
      replace_node(m.get_match_root(),
      ...
      replace_node(m.get_match_root(), make_constant_binary<double>(
                                           a_match, b_match, binary_match));
      return true;
    }
  ...
  auto reshape_matcher = make_shared<pattern::Matcher>(
      bea, constant_binary_callback, "HEConstantFolding.ConstantBinary");
  this->add_matcher(reshape_matcher);
}

bool is_supported_unary_op(std::shared_ptr<Node> n) {
  return std::dynamic_pointer_cast<op::Abs>(n) ||
         std::dynamic_pointer_cast<op::Negative>(n) ||
...
shared_ptr<op::Constant> make_constant_unary(shared_ptr<op::Constant> constant,
  ...
  if (std::dynamic_pointer_cast<op::Abs>(unary)) {
    runtime::reference::abs<T>(constant->get_vector<T>().data(), out_vec.data(),
                               shape_size(out_shape));
  } else if (std::dynamic_pointer_cast<op::Negative>(unary)) {
    runtime::reference::negate<T>(constant->get_vector<T>().data(),
                                  out_vec.data(), shape_size(out_shape));
  } else if (std::dynamic_pointer_cast<op::Relu>(unary)) {
    runtime::reference::relu<T>(constant->get_vector<T>().data(),
                                out_vec.data(), shape_size(out_shape));
  } else if (std::dynamic_pointer_cast<op::Sqrt>(unary)) {
    runtime::reference::sqrt<T>(constant->get_vector<T>().data(),
                                out_vec.data(), shape_size(out_shape));
  } else {
...
void pass::HEConstantFolding::construct_constant_unary() {
  auto constant_label = make_shared<pattern::op::Label>(
      element::f32, Shape{2, 4}, pattern::has_class<op::Constant>());
  auto is_uea = pattern::has_class<op::util::UnaryElementwiseArithmetic>();
  auto uea = std::make_shared<pattern::op::Any>(constant_label, is_uea,
                                                NodeVector{constant_label});
  ...
  auto constant_unary_callback = [constant_label](pattern::Matcher& m) {
    ...
    auto constant_match =
        dynamic_pointer_cast<op::Constant>(pattern_map[constant_label]);
    ...
    if (type == element::i32) {
      replace_node(m.get_match_root(),
                   make_constant_unary<int>(constant_match, unary_match));
      return true;
    } else if (type == element::i8) {
      replace_node(m.get_match_root(),
  ...
  auto reshape_matcher = make_shared<pattern::Matcher>(
      uea, constant_unary_callback, "HEConstantFolding.ConstantUnary");
  this->add_matcher(reshape_matcher);
}

template <class QUANT, class REAL>
shared_ptr<op::Constant> make_constant_dequantize(
    shared_ptr<op::Constant> constant, shared_ptr<op::Dequantize> dequant,
    shared_ptr<op::Constant> scale, shared_ptr<op::Constant> offset) {
  auto out_shape = constant->get_shape();
  vector<REAL> out_vec(shape_size(out_shape));

  runtime::reference::dequantize<QUANT, REAL>(
      constant->get_vector<QUANT>().data(), scale->get_vector<REAL>().data(),
      offset->get_vector<QUANT>().data(), out_vec.data(), constant->get_shape(),
      scale->get_shape(), dequant->get_axes());

  return make_shared<op::Constant>(dequant->get_element_type(), out_shape,
                                   out_vec);
}

void pass::HEConstantFolding::construct_constant_dequantize() {
  auto constant_label = make_shared<pattern::op::Label>(
      element::u8, Shape{2}, pattern::has_class<op::Constant>());
  auto dq_scale = op::Constant::create(element::f32, Shape{}, {1});
  auto dq_offset = op::Constant::create(element::u8, Shape{}, {1});
  auto dequant_op = make_shared<op::Dequantize>(
      constant_label, dq_scale, dq_offset, element::f32, AxisSet{});
  auto dequant = make_shared<pattern::op::Label>(dequant_op, nullptr,
                                                 NodeVector{dequant_op});

  auto constant_dequantize_callback = [constant_label,
                                       dequant](pattern::Matcher& m) {
    NGRAPH_DEBUG
        << "In callback for constant_dequantize_callback against node = "
        << m.get_match_root()->get_name();
  ...
  auto dequantize_matcher =
      make_shared<pattern::Matcher>(dequant, constant_dequantize_callback,
                                    "HEConstantFolding.ConstantDequantize");
  this->add_matcher(dequantize_matcher);
}

template <class REAL, class QUANT>
shared_ptr<op::Constant> make_constant_quantize(
    shared_ptr<op::Constant> constant, shared_ptr<op::Quantize> quant,
    shared_ptr<op::Constant> scale, shared_ptr<op::Constant> offset) {
  auto out_shape = constant->get_shape();
  vector<QUANT> out_vec(shape_size(out_shape));

  runtime::reference::quantize<REAL, QUANT>(
      constant->get_vector<REAL>().data(), scale->get_vector<REAL>().data(),
      offset->get_vector<QUANT>().data(), out_vec.data(), constant->get_shape(),
      scale->get_shape(), quant->get_axes(), quant->get_round_mode());

  return make_shared<op::Constant>(quant->get_element_type(), out_shape,
                                   out_vec);
}

void pass::HEConstantFolding::construct_constant_quantize() {
  auto constant_label = make_shared<pattern::op::Label>(
      element::f32, Shape{2}, pattern::has_class<op::Constant>());
  ...
  auto quant_op = make_shared<op::Quantize>(constant_label, q_scale, q_offset,
                                            element::i8, AxisSet{}, mode);
  auto quant =
      make_shared<pattern::op::Label>(quant_op, nullptr, NodeVector{quant_op});

  auto constant_quantize_callback = [constant_label,
                                     quant](pattern::Matcher& m) {
    ...
    if (type == element::u8) {
      replace_node(m.get_match_root(),
                   make_constant_quantize<float, uint8_t>(
                       constant_match, quantize_op, scale, offset));
      return true;
    } else if (type == element::i8) {
      replace_node(m.get_match_root(),
                   make_constant_quantize<float, int8_t>(
                       constant_match, quantize_op, scale, offset));
      return true;
    }
  ...
  auto quantize_matcher = make_shared<pattern::Matcher>(
      quant, constant_quantize_callback, "HEConstantFolding.ConstantQuantize");
