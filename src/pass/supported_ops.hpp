...
class SupportedOps : public ngraph::pass::FunctionPass {
 public:
  /// \param is_supported Function which returns whether or not a given Node is
  /// supported
  ...
  /// \param function Function which to run pass on
  bool run_on_function(std::shared_ptr<ngraph::Function> function) override;

  /// \brief returns whether or not given node is supported
  /// \param node Node which to check supported status
