                                      ...
                                      shared_ptr<HEBackend> he_backend)
{
    ...
    for (const Coordinate& arg0_projected_coord : arg0_projected_transform)
    {
        for (const Coordinate& arg1_projected_coord : arg1_projected_transform)
        {
            ...
            auto out_coord_it = std::copy(
                arg0_projected_coord.begin(), arg0_projected_coord.end(), out_coord.begin());
            std::copy(arg1_projected_coord.begin(), arg1_projected_coord.end(), out_coord_it);

            // Zero out to start the sum.
            shared_ptr<HECipherTensorView> sum_tv = static_pointer_cast<HECipherTensorView>(
                he_backend->create_zero_tensor(type, Shape{1}));
            shared_ptr<seal::Ciphertext> sum = sum_tv->get_element(0);
            shared_ptr<HECipherTensorView> prod_tv = static_pointer_cast<HECipherTensorView>(
                he_backend->create_zero_tensor(type, Shape{1}));
            ...
            auto arg0_it = std::copy(
                arg0_projected_coord.begin(), arg0_projected_coord.end(), arg0_coord.begin());
            for (const Coordinate& dot_axis_positions : dot_axes_transform)
            {
                ...
                auto arg1_it = std::copy(
                    dot_axis_positions.begin(), dot_axis_positions.end(), arg1_coord.begin());
                std::copy(arg1_projected_coord.begin(), arg1_projected_coord.end(), arg1_it);

                // Multiply and add to the sum.
                ngraph::runtime::he::kernel::multiply(arg0[arg0_transform.index(arg0_coord)],
