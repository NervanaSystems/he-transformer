...
void runtime::he::kernel::subtract(const vector<shared_ptr<he::HEPlaintext>>& arg0,
                              const vector<shared_ptr<he::HECiphertext>>& arg1,
                              vector<shared_ptr<he::HECiphertext>>& out,
                              const element::Type& type,
                              shared_ptr<HEBackend> he_backend,
                                     ...
                                     shared_ptr<HEBackend> he_backend)
{
    ...
    else if(auto he_heaan_backend = dynamic_pointer_cast<he_heaan::HEHeaanBackend>(he_backend))
    {
        ...
        {
            kernel::heaan::scalar_subtract(arg0_heaan, arg1_heaan, out_heaan, type, he_heaan_backend);
            out = dynamic_pointer_cast<he::HECiphertext>(out_heaan);
        }
        else
        {
            throw ngraph_error(
                    "subtract backend is heaan, but arguments or outputs are not HeaanCiphertextWrapper");
        }
    }
    else
    {
...
void runtime::he::kernel::scalar_subtract(const shared_ptr<he::HEPlaintext>& arg0,
                                     ...
                                     shared_ptr<HEBackend> he_backend)
{
    ...
    else if(auto he_heaan_backend = dynamic_pointer_cast<he_heaan::HEHeaanBackend>(he_backend))
    {
        ...
        {
            throw ngraph_error(
                    "subtract backend is heaan, but arguments or outputs are not HeaanPlaintextWrapper");
        }
    }
    else
    {
