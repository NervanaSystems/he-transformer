		...
		shared_ptr<HEBackend>& he_backend)
{
	...
	for (const Coordinate& input_coord : input_transform)
	{
		...
		{
			shared_ptr<HECipherTensorView> const_tv =
				static_pointer_cast<HECipherTensorView>(
						he_backend->create_constant_tensor(type, Shape{1}, i));
			seal::Plaintext dec_val;
			seal::Plaintext dec_i;
			he_backend->decrypt(dec_val, *val);						 // TODO: We are not allowed to decrypt! Pass in one-hot encoded inputs
			he_backend->decrypt(dec_i, *(const_tv->get_element(0))); // TODO: We are not allowed to decrypt! Pass in one-hot encoded inputs
		...
		{
			throw(std::range_error("One-hot: non-integral value in input or value is out of category range"));
		}

		Coordinate one_hot_coord = inject(input_coord, one_hot_axis, one_hot_pos);
		shared_ptr<HECipherTensorView> one_tv =
			static_pointer_cast<HECipherTensorView>(
					he_backend->create_ones_tensor(type, Shape{1}));
