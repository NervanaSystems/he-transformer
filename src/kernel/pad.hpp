...
void pad(const std::vector<std::shared_ptr<S>>& arg0,
  ...
  Coordinate input_start(arg0_shape.size(),
                         0);         // start at (0,0,...,0)
  ...
  // Need to cast these to CoordinateDiff in order to make
  // CoordinateTransform happy.
  ...
  for (const Coordinate& in_coord : input_transform) {
    ...
    std::shared_ptr<S> v = input_transform.has_source_coordinate(in_coord)
