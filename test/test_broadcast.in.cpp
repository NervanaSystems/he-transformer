...
auto broadcast_test =
    [](const Shape& shape_a, const Shape& shape_r, const AxisSet& axis_set,
      ...
      auto t = std::make_shared<op::Broadcast>(a, shape_r, axis_set);
...
NGRAPH_TEST(${BACKEND_NAME}, broadcast_scalar_vector_plain_real_unpacked) {
  broadcast_test(Shape{}, Shape{4}, AxisSet{0}, std::vector<float>{6},
...
NGRAPH_TEST(${BACKEND_NAME}, broadcast_scalar_matrix_plain_real_unpacked) {
  broadcast_test(Shape{}, Shape{2, 2}, AxisSet{0, 1}, std::vector<float>{6},
                 std::vector<float>{6, 6, 6, 6}, false, false, false);
}
...
NGRAPH_TEST(${BACKEND_NAME}, broadcast_scalar_tensor_plain_real_unpacked) {
  broadcast_test(
      Shape{}, Shape{2, 2, 2}, AxisSet{0, 1, 2}, std::vector<float>{6},
...
NGRAPH_TEST(${BACKEND_NAME}, broadcast_scalar_tensor_plain_complex_packed) {
  broadcast_test(Shape{}, Shape{2, 2, 2}, AxisSet{0, 1, 2},
                 std::vector<float>{6},
...
NGRAPH_TEST(${BACKEND_NAME}, broadcast_to_non_existent_axis) {
  ...
  auto a = std::make_shared<op::Parameter>(element::f32, shape_a);
  Shape shape_r{4};
  ASSERT_THROW(auto f = std::make_shared<Function>(
                   std::make_shared<op::Broadcast>(a, shape_r, AxisSet{0, 1}),
                   ParameterVector{a}),
               ngraph_error);
}
