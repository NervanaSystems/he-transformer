...
auto sum_test = [](const ngraph::Shape& in_shape, const ngraph::AxisSet& axis_set,
  ...
  auto a = std::make_shared<ngraph::op::Parameter>(ngraph::element::f32, in_shape);
  ...
  auto t_a =ngraph::test::he::tensor_from_flags(*he_backend, in_shape, arg1_encrypted,
                                         packed);
  ...
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{2, 2}, ngraph::AxisSet{}, std::vector<float>{1, 2, 3, 4},
                 std::vector<float>{1, 2, 3, 4}, arg1_encrypted, complex_packing,
                 packing);
      }
    }
  }
...
NGRAPH_TEST(${BACKEND_NAME}, sum_to_scalar) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{2, 2}, ngraph::AxisSet{0, 1}, std::vector<float>{1, 2, 3, 4},
                 std::vector<float>{10}, arg1_encrypted, complex_packing, packing);
      }
    }
  }
}
...
NGRAPH_TEST(${BACKEND_NAME}, sum_matrix_columns) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{3, 2}, ngraph::AxisSet{0}, std::vector<float>{1, 2, 3, 4, 5, 6},
...
NGRAPH_TEST(${BACKEND_NAME}, sum_matrix_rows) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{3, 2}, ngraph::AxisSet{1}, std::vector<float>{1, 2, 3, 4, 5, 6},
...
NGRAPH_TEST(${BACKEND_NAME}, sum_matrix_cols_zero) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{0, 2}, ngraph::AxisSet{0}, std::vector<float>{}, std::vector<float>{0, 0},
                 arg1_encrypted, complex_packing, packing);
      }
    }
  }
...
NGRAPH_TEST(${BACKEND_NAME}, sum_matrix_vector_zero) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{0}, ngraph::AxisSet{0}, std::vector<float>{}, std::vector<float>{0},
                 arg1_encrypted, complex_packing, packing);
      }
    }
  }
...
NGRAPH_TEST(${BACKEND_NAME}, sum_matrix_to_scalar_zero_by_zero) {
  for (bool arg1_encrypted : std::vector<bool>{false, true}) {
    for (bool complex_packing : std::vector<bool>{false, true}) {
      for (bool packing : std::vector<bool>{false}) {
        sum_test(ngraph::Shape{0, 0}, ngraph::AxisSet{0, 1}, std::vector<float>{}, std::vector<float>{0},
                 arg1_encrypted, complex_packing, packing);
      }
    }
  }
