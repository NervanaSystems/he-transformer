...
static void run_cifar10_benchmark(string backend_name, size_t batch_size = 1) {
  ...
  sw_load_model.start();
  const string filename = "cifar10_" + to_string(batch_size);
  const string json_path =
      file_util::path_join(HE_SERIALIZED_ZOO, filename + ".json");

  NGRAPH_INFO << "Reading file " << json_path;
  ...
  for (const auto& parameter : parameters) {
    ...
    if (shape == Shape{batch_size, 24, 24, 3}) {
...
NGRAPH_TEST(CIFAR10, CKKS_1) { run_cifar10_benchmark("HE:SEAL:CKKS", 1); }
