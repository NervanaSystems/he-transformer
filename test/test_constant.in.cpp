...
NGRAPH_TEST(${BACKEND_NAME}, constant_abc) {
  ...
  {
    auto A = op::Constant::create(element::f64, shape, {1, 2, 3, 4});
    ...
    for (auto tensors : tensors_list) {
      ...
      auto c = inputs[1];
      auto result = results[0];
      copy_data(b, test::NDArray<double, 2>({{5, 6}, {7, 8}}).get_vector());
      copy_data(c, test::NDArray<double, 2>({{9, 10}, {11, 12}}).get_vector());
      auto handle = backend->compile(f);
      handle->call_with_validate({result}, {b, c});
      EXPECT_TRUE(all_close(
          read_vector<double>(result),
