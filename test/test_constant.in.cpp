...
NGRAPH_TEST(${BACKEND_NAME}, constant) {
  ...
  {
    auto A = ngraph::op::Constant::create(ngraph::element::f32, shape, {0.1, 0.2, 0.3, 0.4});
    ...
    EXPECT_TRUE(ngraph::test::he::all_close((std::vector<float>{0.1, 0.2, 0.3, 0.4}),
                                    read_vector<float>(result)));
  }
  {
    auto A = ngraph::op::Constant::create(ngraph::element::f64, shape, {0.1, 0.2, 0.3, 0.4});
    auto f = std::make_shared<ngraph::Function>(A, ngraph::ParameterVector{});
    auto result = backend->create_tensor(ngraph::element::f64, shape);
    auto handle = backend->compile(f);
    handle->call_with_validate({result}, {});
    EXPECT_TRUE(ngraph::test::he::all_close((std::vector<double>{0.1, 0.2, 0.3, 0.4}),
...
NGRAPH_TEST(${BACKEND_NAME}, constant_abc_plain_plain) {
  ...
  ngraph::Shape shape{2, 2};
  auto a = ngraph::op::Constant::create(ngraph::element::f32, shape, {1, 2, 3, 4});
