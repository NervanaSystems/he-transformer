...
TEST(${BACKEND_NAME}, sub_2_3)
{
    ...
    for (auto tensors : tensors_list)
    {
        ...
        copy_he_data(t_a, test::NDArray<int64_t, 2>({{1, 2, 3}, {10, 11, 12}}).get_vector(), backend);
        copy_he_data(t_b, test::NDArray<int64_t, 2>({{7, 8, 9}, {4, 5, 6}}).get_vector(), backend);
        backend->call(f, {t_result}, {t_a, t_b});
        EXPECT_EQ(read_he_vector<int64_t>(t_result, backend),
                  (test::NDArray<int64_t, 2>({{-6, -6, -6}, {6, 6, 6}})).get_vector());
    }
}
