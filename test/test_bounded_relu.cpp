...
TEST(bounded_relu, copy) {
  ...
  NodeVector new_args{std::make_shared<op::Parameter>(element::f32, shape)};

  auto node = std::make_shared<op::BoundedRelu>(arg0, 7.2);
  auto new_node = node->copy_with_new_args(new_args);

  EXPECT_TRUE(new_node != nullptr);
  EXPECT_TRUE(new_args == new_node->get_arguments());

  auto new_bounded_relu = std::static_pointer_cast<op::BoundedRelu>(new_node);
