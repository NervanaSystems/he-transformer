...
NGRAPH_TEST(${BACKEND_NAME}, multiply_2_3) {
  ...
  for (auto tensors : tensors_list) {
    ...
    copy_data(t_a,
              test::NDArray<float, 2>({{2, 2, 3}, {4, 5, 6}}).get_vector());
    copy_data(t_b,
    ...
    EXPECT_TRUE(all_close(
        ...
        (test::NDArray<float, 2>({{14, 16, 27}, {40, 55, 72}})).get_vector(),
        1e-3f));
  }
}

NGRAPH_TEST(${BACKEND_NAME}, multiply_2_3_dummy) {
  auto backend = runtime::Backend::create("${BACKEND_NAME}");
  ...
  copy_data(t_a, test::NDArray<float, 2>({{2, 2, 3}, {4, 5, 6}}).get_vector());
  copy_data(t_b,
            test::NDArray<float, 2>({{7, 8, 9}, {10, 11, 12}}).get_vector());
  auto handle = backend->compile(f);
  handle->call_with_validate({t_result}, {t_a, t_b});
