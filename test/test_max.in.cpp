...
auto max_test = [](const Shape& shape_a, const AxisSet& reduction_axes,
  ...
  auto t = std::make_shared<op::Max>(a, reduction_axes);
...
NGRAPH_TEST(${BACKEND_NAME}, max_trivial_plain_real_unpacked) {
  max_test(Shape{2, 2}, AxisSet{}, std::vector<float>{1, 2, 3, 4},
...
NGRAPH_TEST(${BACKEND_NAME}, max_to_scalar_plain_real_unpacked) {
  max_test(Shape{2, 2}, AxisSet{0, 1}, std::vector<float>{1, 2, 3, 4},
...
NGRAPH_TEST(${BACKEND_NAME}, max_matrix_columns_plain_real_unpacked) {
  max_test(Shape{3, 2}, AxisSet{0}, std::vector<float>{1, 2, 3, 4, 5, 6},
...
NGRAPH_TEST(${BACKEND_NAME}, max_matrix_rows_plain_real_unpacked) {
  max_test(Shape{3, 2}, AxisSet{1}, std::vector<float>{1, 2, 3, 4, 5, 6},
...
NGRAPH_TEST(${BACKEND_NAME}, max_matrix_rows_zero_plain_real_unpacked) {
  max_test(Shape{3, 0}, AxisSet{1}, std::vector<float>{},
...
NGRAPH_TEST(${BACKEND_NAME}, max_matrix_cols_zero_plain_real_unpacked) {
  max_test(Shape{0, 2}, AxisSet{0}, std::vector<float>{},
           std::vector<float>{-std::numeric_limits<float>::infinity(),
                              -std::numeric_limits<float>::infinity()},
           false, false, false);
}
...
NGRAPH_TEST(${BACKEND_NAME}, max_vector_zero_plain_real_unpacked) {
  max_test(Shape{0}, AxisSet{0}, std::vector<float>{},
...
NGRAPH_TEST(${BACKEND_NAME},
  ...
  max_test(Shape{0, 0}, AxisSet{0, 1}, std::vector<float>{},
           std::vector<float>{-std::numeric_limits<float>::infinity()}, false,
           false, false);
}
